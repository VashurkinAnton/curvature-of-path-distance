...
// Dependencies passed as arguments
	...
	Graph.prototype.getShortestPathsMatrix = function(MST) {
		...
		leaves.forEach(function(leaf){
			...
			this.shortestPathsList(leaf, nodes, leaves, availableEdges).forEach(function(shortestPath, index){
				var nodeIndex = this.data.indexOf(nodes[index]);
	...
	Graph.prototype.shortestPathsList = function(fromNode, toNodes, skip, availableEdges){//Dijkstra’s algorithm
		var nodesToCleanup = [];
		var heap = [fromNode];
		...
		var shortestPaths = toNodes.map(function(node){
			return node.path;
		});
